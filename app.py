...
def fetch_nse_chain(symbol):
    """
    Robust NSE/nselib adapter.

    nselib/NSE responses can arrive as:
      1) DataFrame
      2) dict with records/data
      3) dict with nested CE/PE records
      4) list of rows
    The previous build assumed a DataFrame and crashed on a dict with
    the error: KeyError: 'records'. This adapter normalizes all common
    shapes into one CE/PE-per-strike DataFrame.
    """
    try:
        ...
        if raw is None:
            return None, "NSE returned no option chain."
        ...
        # Decode nested NSE/nselib JSON
        payload = raw
        if isinstance(payload, dict):
            records = payload.get("records", payload)

            # Typical NSE structure:
            # {"records": {"data": [...], "underlyingValue": ...}}
            if isinstance(records, dict):
                rows = records.get("data", records.get("records", []))
            else:
                rows = records

            if isinstance(rows, dict):
                # Some wrappers return {"data": {"records": [...]}}
                rows = rows.get("data", rows.get("records", []))

            if not isinstance(rows, list):
                return None, "NSE option-chain response has no usable records."
            ...
            for item in rows:
                ...
                strike = item.get("strikePrice", item.get("strike_price", item.get("Strike Price")))
                expiry = item.get("expiryDate", item.get("expiry_date", ""))

                ce = item.get("CE") or item.get("ce") or {}
                pe = item.get("PE") or item.get("pe") or {}

                # If the API already supplied flat columns, preserve them.
                if ce or pe:
                    flat.append({
                        "Strike Price": strike,
                        "Expiry": expiry,
                        "CALLS_OI": ce.get("openInterest", ce.get("open_interest", ce.get("oi", 0))),
                        "PUTS_OI": pe.get("openInterest", pe.get("open_interest", pe.get("oi", 0))),
                        "CALLS_Chng in OI": ce.get("changeinOpenInterest", ce.get("change_in_oi", ce.get("changeInOI", 0))),
                        "PUTS_Chng in OI": pe.get("changeinOpenInterest", pe.get("change_in_oi", pe.get("changeInOI", 0))),
                        "CALLS_Volume": ce.get("totalTradedVolume", ce.get("volume", 0)),
                        "PUTS_Volume": pe.get("totalTradedVolume", pe.get("volume", 0)),
                        "CALLS_LTP": ce.get("lastPrice", ce.get("ltp", 0)),
                        "PUTS_LTP": pe.get("lastPrice", pe.get("ltp", 0)),
                        "CALLS_IV": ce.get("impliedVolatility", ce.get("iv", 0)),
                        "PUTS_IV": pe.get("impliedVolatility", pe.get("iv", 0)),
                    })
                else:
                    flat.append(item)

            if not flat:
                return None, "NSE returned records, but no strike rows were found."

            df = pd.DataFrame(flat)

            # Keep the first/current expiry if multiple expiries are returned.
            if "Expiry" in df.columns:
                ex = df["Expiry"].replace("", np.nan).dropna()
                if not ex.empty:
                    first_expiry = ex.iloc[0]
                    same = df[df["Expiry"].astype(str) == str(first_expiry)].copy()
                    if not same.empty:
                        df = same

            if df.empty:
                return None, "No current-expiry option rows."
        ...
        return None, f"Unsupported NSE response type: {type(raw).__name__}"

    except Exception as e:
        return None, f"NSE option-chain adapter error: {type(e).__name__}: {e}"
...
def option_analysis(chain, spot):
    ...
    aliases={
        "strike":["Strike Price","strikePrice","strike_price","STRIKE_PRICE"],
        "coi":["CALLS_OI","CE_OI","call_oi","CALL_OI"],
        "poi":["PUTS_OI","PE_OI","put_oi","PUT_OI"],
        "cd":["CALLS_Chng in OI","CE_Chng_OI","call_change_oi","CALL_CHANGE_OI"],
        "pd":["PUTS_Chng in OI","PE_Chng_OI","put_change_oi","PUT_CHANGE_OI"],
        "cv":["CALLS_Volume","CE_Volume","call_volume","CALL_VOLUME"],
        "pv":["PUTS_Volume","PE_Volume","put_volume","PUT_VOLUME"],
        "cltp":["CALLS_LTP","CE_LTP","call_ltp","CALL_LTP"],
        "pltp":["PUTS_LTP","PE_LTP","put_ltp","PUT_LTP"],
    }
    def pick(names):
        for n in names:
...
if opt:
    ...
    st.caption("No options score is fabricated. The Options/OI module remains PENDING. Live NSE option-chain snapshots are expected during market hours; outside market hours the live layer may legitimately be unavailable.")
